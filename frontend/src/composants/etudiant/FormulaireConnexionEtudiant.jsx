...
const FormulaireInscriptionEtudiant = ({ onSuccess }) => {
  ...
  // État pour gérer le modal de validation
  ...
  const handleSubmit = async (e) => {
    ...
    // Si l'inscription réussit, ouvrir le modal de validation
    if (resultat.success) {
      setEmailInscrit(formData.email);
      setShowModalValidation(true);
    }
    // Si échec, les erreurs sont déjà gérées par le hook
  };

  // Appelé uniquement quand le code de validation est validé avec succès
  const handleValidationSuccess = () => {
    setShowModalValidation(false);
    // Informer le composant parent que tout le processus est terminé
    if (onSuccess) {
      onSuccess({ email: emailInscrit });
    }
  };
