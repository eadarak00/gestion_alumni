/*
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech) (7.17.0).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
package uasz.alumni.ms_cv.controllers;

import uasz.alumni.ms_cv.models.Formation;
import uasz.alumni.ms_cv.models.FormationCreate;
import io.swagger.v3.oas.annotations.ExternalDocumentation;
import io.swagger.v3.oas.annotations.Operation;
import io.swagger.v3.oas.annotations.Parameter;
import io.swagger.v3.oas.annotations.Parameters;
import io.swagger.v3.oas.annotations.media.ArraySchema;
import io.swagger.v3.oas.annotations.media.Content;
import io.swagger.v3.oas.annotations.media.Schema;
import io.swagger.v3.oas.annotations.responses.ApiResponse;
import io.swagger.v3.oas.annotations.security.SecurityRequirement;
import io.swagger.v3.oas.annotations.tags.Tag;
import io.swagger.v3.oas.annotations.enums.ParameterIn;
import io.swagger.v3.oas.annotations.media.ExampleObject;
import org.springframework.http.HttpStatus;
import org.springframework.http.MediaType;
import org.springframework.http.ResponseEntity;
import org.springframework.validation.annotation.Validated;
import org.springframework.web.bind.annotation.*;
import org.springframework.web.context.request.NativeWebRequest;
import org.springframework.web.multipart.MultipartFile;

import javax.validation.Valid;
import javax.validation.constraints.*;
import java.util.List;
import java.util.Map;
import java.util.Optional;
import javax.annotation.Generated;

@Generated(value = "org.openapitools.codegen.languages.SpringCodegen", date = "2025-11-26T15:28:11.703869118Z[Africa/Dakar]", comments = "Generator version: 7.17.0")
@Validated
@Tag(name = "Formations", description = "the Formations API")
public interface FormationsApi {

    default Optional<NativeWebRequest> getRequest() {
        return Optional.empty();
    }

    String PATH_FORMATIONS_ID_DELETE = "/formations/{id}";
    /**
     * DELETE /formations/{id} : Supprimer une formation
     *
     * @param id  (required)
     * @return Formation supprimée (status code 204)
     */
    @Operation(
        operationId = "formationsIdDelete",
        summary = "Supprimer une formation",
        tags = { "Formations" },
        responses = {
            @ApiResponse(responseCode = "204", description = "Formation supprimée")
        }
    )
    @RequestMapping(
        method = RequestMethod.DELETE,
        value = FormationsApi.PATH_FORMATIONS_ID_DELETE
    )
    default ResponseEntity<Void> formationsIdDelete(
        @NotNull @Parameter(name = "id", description = "", required = true, in = ParameterIn.PATH) @PathVariable("id") Integer id
    ) {
        return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);

    }


    String PATH_FORMATIONS_ID_PUT = "/formations/{id}";
    /**
     * PUT /formations/{id} : Modifier une formation
     *
     * @param id  (required)
     * @param formationCreate  (required)
     * @return Formation mise à jour (status code 200)
     */
    @Operation(
        operationId = "formationsIdPut",
        summary = "Modifier une formation",
        tags = { "Formations" },
        responses = {
            @ApiResponse(responseCode = "200", description = "Formation mise à jour")
        }
    )
    @RequestMapping(
        method = RequestMethod.PUT,
        value = FormationsApi.PATH_FORMATIONS_ID_PUT,
        consumes = { "application/json" }
    )
    default ResponseEntity<Void> formationsIdPut(
        @NotNull @Parameter(name = "id", description = "", required = true, in = ParameterIn.PATH) @PathVariable("id") Integer id,
        @Parameter(name = "FormationCreate", description = "", required = true) @Valid @RequestBody FormationCreate formationCreate
    ) {
        return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);

    }


    String PATH_FORMATIONS_POST = "/formations";
    /**
     * POST /formations : Ajouter une formation
     *
     * @param formationCreate  (required)
     * @return Formation ajoutée (status code 201)
     *         or Données invalides (status code 400)
     */
    @Operation(
        operationId = "formationsPost",
        summary = "Ajouter une formation",
        tags = { "Formations" },
        responses = {
            @ApiResponse(responseCode = "201", description = "Formation ajoutée", content = {
                @Content(mediaType = "application/json", schema = @Schema(implementation = Formation.class))
            }),
            @ApiResponse(responseCode = "400", description = "Données invalides")
        }
    )
    @RequestMapping(
        method = RequestMethod.POST,
        value = FormationsApi.PATH_FORMATIONS_POST,
        produces = { "application/json" },
        consumes = { "application/json" }
    )
    default ResponseEntity<Formation> formationsPost(
        @Parameter(name = "FormationCreate", description = "", required = true) @Valid @RequestBody FormationCreate formationCreate
    ) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
                    String exampleString = "{ \"cvId\" : 1, \"dateDebut\" : \"2015-10-01\", \"localisation\" : \"Ziguinchor, Sénégal\", \"description\" : \"Formation en développement logiciel\", \"etablissement\" : \"UASZ\", \"id\" : 1, \"dateFin\" : \"2018-07-30\", \"diplome\" : \"Licence en Informatique\", \"enCours\" : false }";
                    ApiUtil.setExampleResponse(request, "application/json", exampleString);
                    break;
                }
            }
        });
        return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);

    }

}
